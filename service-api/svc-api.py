...
app = Flask(__name__)
api = Api(app, version='1.0', title='Services API', 
          description='Service API for managing EN Infrastructure in BRKOPS-2357')

sites = api.namespace('site', 
                   description='site management')

vpns = api.namespace('vpn', 
                   description='vpn management')

infra = api.namespace('infra', 
...
class ServiceDb():
    ...
    def post_vpn(self,id=int,name=str,sites=list):
        ...
        response = requests.post(f"{self.url}/vpn-service:vpns", data=json.dumps(payload))
        if response.status_code != 200:
            return {"error": response.content.decode('utf-8')}
    ...
    def patch_vpn(self,id=int,name=str,sites=list):
        ...
        response = requests.patch(f"{self.url}/vpn-service:vpns={id}", data=json.dumps(payload))
        print(response)
        if response.status_code != 200:
            return {"error": response.content.decode('utf-8')}
        return response.status_code
    ...
    def post_site(self,id=int,name=str,type=str,router=list,switches=list):
        payload = {"sites": [{"id":id, "name": name,"type": type, "router": router, "switches": switches }]}
        response = requests.post(f"{self.url}/site-service:sites", data=json.dumps(payload))
        if response.status_code != 200:
            return {"error": response.content.decode('utf-8')}
        return response.status_code
    ...
    def patch_site(self,id=int,name=str,type=str,router=list,switches=list):
        ...
        response = requests.patch(f"{self.url}/site-service:sites={id}", data=json.dumps(payload))
        if response.status_code != 200:
            return {"error": response.content.decode('utf-8')}
        return response.status_code
...
class Git:
    ...
    def __create_branch(self):
        ...
        self.new_branch = self.project.branches.create({'branch': branch_name, 'ref': 'main'})
    ...
    def __commit_to_branch(self, site_service_data, vpn_service_data):     
        ...
        site_service_file = self.project.files.get(file_path=site_service_file_path, ref=self.new_branch.name)
        vpn_service_file = self.project.files.get(file_path=vpn_service_file_path, ref=self.new_branch.name)
        site_service_file_content = base64.b64decode(site_service_file.content).decode("utf-8")
        vpn_service_file_content = base64.b64decode(vpn_service_file.content).decode("utf-8")

        if site_service_file_content != site_service_data:
            site_service_file.content = site_service_data
            site_service_file.save(branch=self.new_branch.name, commit_message='site service update by service-api', author_email='service-api@its-best.ch', author_name='service-api')

        if vpn_service_file_content != vpn_service_data:
            vpn_service_file.content = vpn_service_data
            vpn_service_file.save(branch=self.new_branch.name, commit_message='vpn service update by service-api', author_email='service-api@its-best.ch', author_name='service-api')
    ...
    def push_to_infra(self):
        ...
        self.__commit_to_branch(site_service_data=site_service_json,vpn_service_data=vpn_service_json)
...
vpn_post_parser.add_argument("sites", type=str, required=True, choices=("DC", "Branch"), action="append", help="DC and/or Branch")

vpn_patch_parser = reqparse.RequestParser()
vpn_patch_parser.add_argument("name", type=str, required=False, help="VPN Name")
vpn_patch_parser.add_argument("sites", type=str, required=False, choices=("DC", "Branch"), action="append", help="DC and/or Branch")
...
site_post_parser.add_argument("type", type=str, required=True, choices=("DC", "Branch"), help="DC or Branch")
site_post_parser.add_argument("router", type=str, required=True, action="append", help="Router serial numbers")
site_post_parser.add_argument("switches", type=str, required=True, action="append", help="Switch serial numbers")

site_patch_parser = reqparse.RequestParser()
site_patch_parser.add_argument("name", type=str, required=False, help="Site Name")
site_patch_parser.add_argument("type", type=str, required=False, choices=("DC", "Branch"), help="DC or Branch")
site_patch_parser.add_argument("router", type=str, required=False, action="append", help="Router serial numbers")
site_patch_parser.add_argument("switches", type=str, required=False, action="append", help="Switch serial numbers")
...
class AllSites(Resource):        
    ...
    def post(self):
        ...
        return services.post_site(id=args.id,name=args.name,type=args.type,router=args.router,switches=args.switches)
...
class SiteId(Resource):        
    ...
    def patch(self,id):
        ...
        return services.patch_site(id=id,name=args.name,type=args.type,router=args.router,switches=args.switches)
